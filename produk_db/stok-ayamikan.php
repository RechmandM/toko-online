...
if (isset($_POST['submit'])) {
    $row = mysqli_query($conn, "SELECT * from ayamikan");
    foreach ($row as $row_array) {
        $idku = $row_array['id'];
        // $stok = $_POST['stok'];
        $ambil = $_POST["ambil" . "$row_array[id]"];
        // $ambil = $_POST["ambil$row_array[id]"];
        mysqli_query($conn, "UPDATE ayamikan set stok='$ambil' where id='$idku'");
    }
    $kategori = "ayamikan";
    ...
    echo "<script>window.location = 'admin.php?mi=ayamikan'</script>";
