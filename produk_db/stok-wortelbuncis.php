...
<section id="admin" class="pt-5 text-center">
    <div class="container pt-5">
        <div class="">
            <div class="pt-5">
                <h3 class="fw-bold text-primary">Wortel, Buncis & Lainnya</h3>
                <hr>
                <div class="card py-2 mb-3">
                    <?php
                    $row = mysqli_query($conn, "SELECT * From tanggal where kategori='wortelbuncis'");
    ...
    <div class="container" data-aos="fade">
        <div>
            <form method="POST" class="needs-validation" novalidate style="font-size: larger;">
                <div class="row">
                    <?php
                    $row = mysqli_query($conn, "SELECT * From wortelbuncis order by stok DESC, tipe, nama");
                    $img = 1;
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="row">
                                <div class="col-3 card">
                                    <img src="./img/produk/wortelbuncis/<?= $row_array['id'] ?>.jpg" class="card-img-top" alt="..." width="auto">
...
if (isset($_POST['submit'])) {
    $row = mysqli_query($conn, "SELECT * from wortelbuncis");
    foreach ($row as $row_array) {
        $idku = $row_array['id'];
        // $stok = $_POST['stok'];
        $ambil = $_POST["ambil" . "$row_array[id]"];
        // $ambil = $_POST["ambil$row_array[id]"];
        mysqli_query($conn, "UPDATE wortelbuncis set stok='$ambil' where id='$idku'");
    }
    $kategori = "wortelbuncis";
    ...
    echo "<script>window.location = 'admin.php?mi=wortelbuncis'</script>";
